...
const insertOrder = (id, username, productName, price, quantity, address, label, category) => {
  const sql = `
    INSERT INTO orders (id, username, product_name, price, quantity, order_status, address, label, category)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
...
const getOrdersBySeller = async (sellerId) => {
  const sql = `SELECT o.* FROM orders o JOIN products p ON o.product_name = p.name WHERE p.addedBy =? ORDER BY order_date DESC;`
